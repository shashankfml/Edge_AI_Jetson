...
try:
    vehicle_model = torch.hub.load('./yolov5', 'custom', source='local', 
                                 path='/home/fmlpc/app_project/vehicle_detection/exp_v2/weights/best.pt')
    ...
    for detection in vehicle_results.xyxy[0]:
        ...
        for plate_det in plate_results.xyxy[0]:
            ...
            try:
                ...
                ocr_results = reader.readtext(processed_plate, detail=1)
                if ocr_results:
                    # Get result with highest confidence
                    best_result = max(ocr_results, key=lambda x: x[2])
                    if best_result[2] > 0.7:  # OCR confidence threshold
        ...
        if tracker.is_duplicate(vehicle_center, license_number):
            continue
            
        # Update tracker and write to CSV
        tracker.update(f"{x1}_{y1}_{x2}_{y2}", vehicle_center, license_number)
